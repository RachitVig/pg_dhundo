...
import { Globe, Bell, PlusCircle, Zap, ShieldCheck, User, X } from 'lucide-react';
    ...
    <nav className="sticky top-0 z-[2000] bg-white/90 backdrop-blur-xl border-b border-slate-200/50 px-6 py-4">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
           {currentUser ? (
              <div className="relative">
                 <button 
                  ...
                  className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-900 font-black border border-slate-200 hover:bg-slate-200 transition-all"
                 >
                    {currentUser.name[0]}
                 </button>
                 <AnimatePresence>
                    {isProfileOpen && (
                       <motion.div 
                        ...
                        className="absolute right-0 mt-4 w-48 bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden z-[3000]"
                       >
           ...
           <button onClick={() => setIsOwnerModalOpen(true)} className="hidden sm:flex px-4 py-2 text-[10px] font-black uppercase text-blue-600 border-2 border-blue-50 bg-blue-50/50 rounded-xl hover:bg-blue-100 transition-colors">Add Property</button>
